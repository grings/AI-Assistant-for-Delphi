...
type
  ...
  private
    ...
    procedure SendOpenAICompatible(const APrompt, AEndpoint, AAPIKey, AModel: string;
      ACallback: TAIResultCallback);
    ...
    procedure SendOpenAICompatibleChat(const AHistory: TArray<TChatMessage>;
      const AEndpoint, AAPIKey, AModel: string; ACallback: TAIResultCallback);
    ...
    function BuildChatOpenAIJSON(const AHistory: TArray<TChatMessage>;
...
const
  SYSTEM_PROMPT =
    'You are an expert Delphi/Pascal developer. ' +
    'Return ONLY the raw Delphi/Pascal source code — no markdown, no code fences, ' +
    'no ``` markers, no explanations before or after the code. ' +
...
begin
  ...
  try
    ...
    OllamaSystem.AddPair('content',
      'You are an expert Delphi/Pascal developer. Return ONLY Pascal source code.');
...
const
  SYSTEM_PROMPT =
    ...
    'opening fence contains the suggested filename, for example:' + #13#10 +
    '```pascal MyUnit.pas' + #13#10 +
    '...code...' + #13#10 +
    '```' + #13#10 +
...
function TAIClient.BuildChatOpenAIJSON(const AHistory: TArray<TChatMessage>;
  const AModel: string): string;
const
  SYSTEM_CONTENT =
    'You are an expert Delphi/Pascal developer. ' +
    'When generating files, wrap each file in a fenced code block whose ' +
    'opening fence contains the suggested filename, for example: ' +
    '```pascal MyUnit.pas ... ```. ' +
    'Supported extensions: .pas .dpr .dpk .dfm .dproj .groupproj .ini .txt. ' +
    'Use 2-space indentation. Follow Delphi naming conventions.';
var
...
const
  SYSTEM_CONTENT =
    'You are an expert Delphi/Pascal developer. ' +
    'When generating files, wrap each file in a fenced code block with a filename hint. ' +
    'Use 2-space indentation. Follow Delphi naming conventions.';
var
  Root      : TJSONObject;
  Messages  : TJSONArray;
...
begin
  ...
  try
    ...
    begin
      Line := Trim(Lines[I]);
      if (StartIdx < 0) and (Copy(Line, 1, 3) = '```') then begin StartIdx := I; Continue; end;
      if (StartIdx >= 0) and (Line = '```') then EndIdx := I;
    end;
    ...
    begin
      ...
      try
        ...
        begin
          SB.Append(Lines[I]);
          if I < EndIdx - 1 then SB.AppendLine;
        end;
        Result := SB.ToString;
      finally
        SB.Free;
...
begin
  if ARole = crUser then Result := 'user' else Result := 'assistant';
end;
...
begin
  ...
  try
    Headers := [
      ...
      TNameValuePair.Create('x-api-key',         GSettings.ClaudeAPIKey),
    ...
    try
      try
        ...
        on E: Exception do
          if not FCancelled then ErrorText := E.Message;
      end;
    finally
      RequestBody.Free;
    end;
  ...
  TThread.Synchronize(nil, procedure begin ACallback(ResultText, ErrorText); end);
end;
...
begin
  ...
  try
    Headers := [
      TNameValuePair.Create('Content-Type',  'application/json'),
      TNameValuePair.Create('Authorization', 'Bearer ' + GSettings.OpenAIAPIKey)
    ];
  ...
  try
    ...
    try
      try
        ...
        on E: Exception do
          if not FCancelled then ErrorText := 'Ollama error (is it running?): ' + E.Message;
      end;
    finally
      RequestBody.Free;
    end;
...
procedure TAIClient.SendOpenAICompatible(const APrompt, AEndpoint, AAPIKey, AModel: string;
  ACallback: TAIResultCallback);
var
  HTTP       : THTTPClient;
  RequestBody: TStringStream;
...
begin
  ...
  begin
    case GSettings.Provider of
      apClaude:  SendClaude(APrompt, ACallback);
      apOpenAI:  SendOpenAI(APrompt, ACallback);
      apOllama:  SendOllama(APrompt, ACallback);
      apGroq:    SendOpenAICompatible(APrompt,
                   GSettings.GroqEndpoint, GSettings.GroqAPIKey,
                   GSettings.GroqModel, ACallback);
      apMistral: SendOpenAICompatible(APrompt,
                   GSettings.MistralEndpoint, GSettings.MistralAPIKey,
                   GSettings.MistralModel, ACallback);
    end;
  end);
end;
...
begin
  Done := False;
  SendAsync(APrompt, procedure(const AResult, AErr: string)
  begin
    ResultStr := AResult;
    ErrorStr  := AErr;
    Done      := True;
  end);
  while not Done do Sleep(50);
  AError := ErrorStr;
  Result := ResultStr;
end;
...
procedure TAIClient.SendClaudeChat(const AHistory: TArray<TChatMessage>;
  ACallback: TAIResultCallback);
var
  HTTP       : THTTPClient;
  RequestBody: TStringStream;
...
procedure TAIClient.SendOpenAIChat(const AHistory: TArray<TChatMessage>;
  ACallback: TAIResultCallback);
var
  HTTP       : THTTPClient;
  RequestBody: TStringStream;
...
begin
  ...
  try
    ...
    RequestBody := TStringStream.Create(
      BuildChatOpenAIJSON(AHistory, GSettings.OpenAIModel), TEncoding.UTF8);
    try
      try
        Response   := HTTP.Post(GSettings.OpenAIEndpoint, RequestBody, nil, Headers);
...
procedure TAIClient.SendOllamaChat(const AHistory: TArray<TChatMessage>;
  ACallback: TAIResultCallback);
var
  HTTP       : THTTPClient;
  RequestBody: TStringStream;
...
procedure TAIClient.SendOpenAICompatibleChat(const AHistory: TArray<TChatMessage>;
  const AEndpoint, AAPIKey, AModel: string; ACallback: TAIResultCallback);
var
  HTTP       : THTTPClient;
  RequestBody: TStringStream;
...
begin
  ...
  try
    ...
    RequestBody := TStringStream.Create(
      BuildChatOpenAIJSON(AHistory, AModel), TEncoding.UTF8);
    try
      try
        Response   := HTTP.Post(AEndpoint, RequestBody, nil, Headers);
        ResultText := ExtractOpenAIResponse(Response.ContentAsString(TEncoding.UTF8));
...
procedure TAIClient.SendChatAsync(const AHistory: TArray<TChatMessage>;
  ACallback: TAIResultCallback);
begin
  TTask.Run(procedure
  begin
    case GSettings.Provider of
      apClaude:  SendClaudeChat(AHistory, ACallback);
      apOpenAI:  SendOpenAIChat(AHistory, ACallback);
      apOllama:  SendOllamaChat(AHistory, ACallback);
      apGroq:    SendOpenAICompatibleChat(AHistory,
                   GSettings.GroqEndpoint, GSettings.GroqAPIKey,
                   GSettings.GroqModel, ACallback);
      apMistral: SendOpenAICompatibleChat(AHistory,
                   GSettings.MistralEndpoint, GSettings.MistralAPIKey,
                   GSettings.MistralModel, ACallback);
    end;
  end);
